...
from model_persisting_split import GCA, Grid, composite  # adjust to match wherever you saved that file
import random
...
GRID_SIZE = 40
OVERLAY_CHANNELS = 16   # was CHANNELS -- this is now the overlay plane's channel count only
GRID_CHANNELS = 1       # occupancy field: 0 = free, 1 = obstructed

OBSTACLES = False        # flip to True for the "grow around obstacles" scenario
OBSTACLE_COUNT = 4
OBSTACLE_RADIUS = (3, 6)
OBSTACLE_MARGIN = 8      # keep this radius clear around the centre seed point
...
def build_grid(grid_size, device=None):
    """Builds the static grid plane according to the OBSTACLES toggle above."""
    if OBSTACLES:
        return Grid.random_obstacles(
            grid_size,
            count=OBSTACLE_COUNT,
            min_radius=OBSTACLE_RADIUS[0],
            max_radius=OBSTACLE_RADIUS[1],
            margin=OBSTACLE_MARGIN,
            device=device,
        )
    return Grid.blank(grid_size, channels=GRID_CHANNELS, device=device)


def visualise(imgTensor, filenameBase="test", anim=False, save=True, show=True, grid=None):
    """
    Visualise a designated snapshot of the grid specified by idx
    Input in form (channels, height, width)

    If `grid` is supplied (a Grid instance), the RGB panel renders
    composite(grid, overlay) instead of the overlay's raw RGB, so obstacles
    (if any) are visible underneath whatever the overlay has grown.
    """
    ...
    def update(imgIdx):
        plt.clf()
        overlay_frame = imgTensor[imgIdx].clip(0, 1)  # (channels, H, W)

        if anim:
            plt.suptitle("Update " + str(imgIdx))

        if grid is not None:
            rgb_img = composite(grid.tensor, overlay_frame.unsqueeze(0))[0].permute(1, 2, 0)
            rgb_title = "Composite (grid + overlay)"
        else:
            rgb_img = overlay_frame.permute(1, 2, 0)[:, :, 0:3]
            rgb_title = "RGB"

        # Plot RGB / composite channels
        plt.subplot(1, 2, 1)
        plt.imshow(rgb_img.detach().numpy())
        plt.title(rgb_title)

        # Plot Alpha channel (the overlay's own aliveness, unaffected by the grid)
        plt.subplot(1, 2, 2)
        plt.imshow(overlay_frame.permute(1, 2, 0)[:, :, 3].detach().numpy())
...
def new_seed(batch=1):
    """
    Creates a 4D tensor with dimensions batch_size x OVERLAY_CHANNELS x GRID_SIZE x GRID_SIZE
    There is a single 1 in the alpha channel of center cell on each grid in the batch.
    This seeds the OVERLAY plane only -- the grid plane is built once via build_grid()
    and never reseeded.
    """
    if not isinstance(batch, Tensor):
        seed = torch.zeros(batch, OVERLAY_CHANNELS, GRID_SIZE, GRID_SIZE)
...
def forward_pass(model: nn.Module, state, updates, record=False): 
    ...
    Uses the module-level GRID as the static plane on every step. GRID is set
    up once in __main__ (and rebuilt only if GRID_SIZE changes for eval).
    """
    if record:
        frames_array = Tensor(updates, OVERLAY_CHANNELS, GRID_SIZE, GRID_SIZE)
        for i in range(updates):
            state = model(GRID.tensor, state)
...
def update_pass(model, batch, target, optimiser, updates_range):
    ...
    ## apply pixel-wise MSE loss between RGBA channels in the overlay and the target pattern
    ## (loss is against the overlay's own channels, not the composite -- the grid is an
    ## architectural constraint, not something we want to grade colour-matching against)
...
if __name__ == "__main__":
    ...
    MODEL = GCA(overlay_channels=OVERLAY_CHANNELS, grid_channels=GRID_CHANNELS)
    MODEL = initialiseGPU(MODEL)

    device = next(MODEL.parameters()).device

    # Build the static grid plane once. It is never reseeded or mutated --
    # only the overlay (the seed / pool samples) evolves during training.
    GRID = build_grid(GRID_SIZE, device=device)
    print(f"Grid built ({'with obstacles' if OBSTACLES else 'blank'}), shape {GRID.shape}")
    ...
    if TRAINING:
        ...
        if (LOAD_WEIGHTS):
            anim = visualise(recording2, anim=True, filenameBase="pool", show=False, grid=GRID)
        else :
            anim = visualise(torch.cat((recording1, recording2), dim=0), anim=True, filenameBase="pool", show=False, grid=GRID)
    ...
    # The static grid must match the overlay's spatial size for every forward
    # call, so it has to be rebuilt at the new GRID_SIZE before this final
    # eval block. Note this generates a FRESH obstacle layout at the larger
    # size rather than rescaling the training layout -- fine for a demo, but
    # build_grid() with a fixed seed/list of obstacles if you need continuity
    # between the training and eval layouts.
    GRID = build_grid(GRID_SIZE, device=device)

    ## Plot final state of evaluation OR evaluation animation
    img = new_seed(1)
    video = forward_pass(MODEL, img, 600, record=True)
    anim = visualise(video, filenameBase = "train", anim=True, grid=GRID)
    anim = visualise(video[-1].unsqueeze(0), filenameBase = "train", anim=False, grid=GRID)
