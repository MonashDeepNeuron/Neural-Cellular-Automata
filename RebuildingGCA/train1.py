...
def save_weights(model, filepath="20_8_model_weights"):
    # NUM_WEIGHTS_0 = 128
    # NUM_BIAS_0 = 128
    # NUM_WEIGHTS_2 = 16
...
def save_model(model, filepath="20_8_model_weights.pth"):
    torch.save(model.state_dict(), filepath)

    print(f"Model weights saved to {filepath}")


def load_model(model_class, filepath="20_8_model_weights.pth", device="cpu"):
...
def setup_tensorboard(logdir="logs"):
    return SummaryWriter(logdir)


if __name__ == "__main__":
    model = GrowingCA(
        n_channels=16, hidden_channels=128, device=torch.device("cuda")
    ).to(torch.device("cuda"))
    tensorboard_writer = setup_tensorboard("logs")
    model = train()
    save_weights(model)
    save_model(model)

    # loaded_model = load_model(GrowingCA, filepath="model_weights.pth", device="cpu")
    # save_weights(loaded_model)
