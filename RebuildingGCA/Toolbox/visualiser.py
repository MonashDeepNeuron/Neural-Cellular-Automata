...
def visualiseHidden(imgTensor, channels_idxs = [], filenameBase="test", columns=4):
    """
    Visualise a designated snapshot of the grid specified by idx
    ...
    for i in range(len(channels_idxs)):
        ...
        canvases.append(plt.imshow((imgTensor[0].squeeze().permute(1, 2, 0))[:, :, channels_idxs[i]].detach().numpy(), cmap="twilight_shifted"))
    ...
    def update(imgIdx):
        ...
        img[0:3, 0, :] = torch.Tensor([-ch_mag, 0, ch_mag]).unsqueeze(1) 
