...
class GrowingCA(nn.Module):
    ...
    def __init__(self, n_channels=16, hidden_channels=128, scale_factor=1, device=None):
        ...
        filters = torch.stack([self.identity, self.sobel_x, self.sobel_y]).repeat(
            (n_channels, 1, 1)
        )
        ...
        self.filters = (
            filters[:, None, ...].type(torch.FloatTensor).to(self.device)
        )  # send these hardcoded filters to the device
        ...
        self.update_step = nn.Sequential(
            nn.Conv2d(
                3
                * n_channels,  # We have 3 * n_channels as we are apply 3 kernels to each channel (perception step)
                hidden_channels,
    ...
    def perceive(self, x):
        ...
        return nn.functional.conv2d(
            x, self.filters, padding=1, groups=self.n_channels
        )  # we apply padding to keep dimensionality
    ...
    def is_alive_mask(x):
        ...
        return (
            ...
            > 0.1
        )  # remember the criterion uses alpha (4th channel)
