...
class CustomDataset(Dataset):
    def __init__(self, images_folder, trimaps_folder, channels = 64, img_channels = 3, transform=None):
...
def load_datasets(images_folder, trimaps_folder, transform, channels, input_channels):
    ...
    dataset = CustomDataset(images_folder, trimaps_folder, transform=transform, channels= channels, img_channels = input_channels)
...
def update_pass(model1, model2, batch, targets, optimiser1, optimiser2):
    """
    ...
    output_patch, patch_target = forward_pass(model1, model2, batch, targets, updates)
...
if __name__ == "__main__":
    ...
    patch_output, patch_target = forward_pass(MODEL1, MODEL2, data, target, updates=UPDATES_RANGE[0])
