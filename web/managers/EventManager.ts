...
export default class EventManager {
	...
	static updateSpeed(inputSpeed) {
		...
		document.getElementById('framesDisplayed').innerHTML =
			`Displays every <b>${EventManager.framesPerUpdateLoop}</b> updates (updates per sec: ${Math.round((1000 * EventManager.framesPerUpdateLoop) / EventManager.updateInterval)})`;
	}
