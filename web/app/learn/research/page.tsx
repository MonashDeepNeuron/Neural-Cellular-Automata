		...
		<div className='max-w-4xl mx-auto px-6 py-10 text-gray-800'>
			...
			<section className='mb-8'>
				...
				<p className = "py-3">
					...
					by graph connections. This allows those textures to seamlessly wrap around 3D objects. In the demonstrations from
					this paper, there are still some flaws in the sharpest corners, however overall the resultant texture is smooth and continuous.
				</p>
			</section>
			...
			<section className='mb-8'>
				...
				<p className = "py-3">This paper takes NCA into the video generation realm. Also based off <i>Self Organising Textures</i> this
				paper aimed to create dynamic directable video-like textures. While this 
				technique was not suited for fast motion scenes, it performed well for scenarios such as fire and 
				rising smoke, suitable for background textures.</p>
			</section>
			...
			<section className='mb-8'>
				...
				<h4 className='italic text-gray-600'>John Kalkhof, Camila González & Anirban Mukhopadhyay  <br/></h4>
				<p className = "py-3">Med-NCA is an image segmentation model, which builds off another proceeding paper <i>Image segmentation via Cellular Automata</i> (Sandler M. et al)
				and is a part of a series of papers that investigate Neural Cellular Automata as a low-resource medical segmentation model. Med-NCA is the first paper to pose NCA as a low memory solution for image segmentation, due to the small model sizes,
				and demonstrates than NCA can effectively segmentation medical grade images while running on Raspberry Pi Zero. 
				</p>
				<p className = "py-3">
				...
				which highlights the issue that NCAs struggle with long-distance communication of information across images, as this
				architectural change sped up the image segmentation process drastically.
				</p>
				<p className = "py-3">
				The authors follow-up this paper by taking NCA to 3D medical scans in <i>M3D-NCA, Robust 3D Segmentation with built in quality control</i>.
				</p>
			</section>
		</div>
	);
